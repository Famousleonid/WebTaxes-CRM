    ...
    <div class="container">
                ...
                <div style="width: 100%; padding: 5px; border: 1px solid #CED4DA;">
                    <div class="container row align-items-center mt-1" style="background-color: #F4F6F9;">
                        <div class="offset-1 col-5 mt-1" style="border: 1px dashed lightblue">
                            <form method="POST" id="reports" enctype="multipart/form-data" action="{{route('addMediaScan', ['firmId'=> $firm->id ])}}">
                                @csrf
                                <input id="filepond-scan" type="file" class="filepond" data-allow-reorder="true"
                                       data-max-file-size="4MB"
                                       name="reports">
                                <input type="text" name="name_doc" value="reports" hidden>
                            </form>
                        </div>
                        <div class="col-6 text-center">
                            ...
                            <button id="spinner-btn" form="reports" class="btn btn-primary align-item-center disabled upload-to-storage upload-maindocument"><span id="spinner-loading" class="spinner-border spinner-border-sm d-none"></span>&nbsp;&nbsp;Upload to storage</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="container col-md-12  mb-5 p-1 bg-white shadow firm-border card-body ">
            @if($reports && $reports->count())
                ...
                <table id="report-tbl" class="table  table-invoices table-bordered table-hover w-100 mt-4">
                    <thead>
                    <tr>
...
@section('scripts')
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let mainTable = $('#report-tbl').DataTable({
                "AutoWidth": true,
                "scrollY": "600px",
                "scrollX": false,
                "scrollCollapse": true,
